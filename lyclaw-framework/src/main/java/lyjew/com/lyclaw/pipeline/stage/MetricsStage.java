...
public class MetricsStage extends PipelineStageBase {
    ...
    public MetricsStage(@org.springframework.lang.Nullable MetricsCollector metricsCollector) {
    ...
    public Flux<ServerSentEvent<String>> execute(AgentContext ctx) {
        ...
        return Flux.defer(() -> {
            ...
            // TODO: 记忆系统重新设计后恢复记忆摄入
            // PerceptionData perception = PerceptionData.builder()
            //         .role("assistant")
            //         .content("编排完成 | 任务: " + taskCount + " | 成功: " + successCount
            //                 + " | 失败: " + failCount + " | 反思评分: " + score)
            //         .timestamp(System.currentTimeMillis())
            //         .metadata(Map.of("sessionId", sessionId, "taskCount", taskCount,
            //                 "successCount", successCount, "failCount", failCount))
            //         .build();
            // memorySystem.ingestPerception(sessionId, perception);
            ...
            return Flux.just(
                    sseEvent("respond_complete", "响应已生成"),
