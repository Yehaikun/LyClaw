...
`enabled` 字段控制记忆是否被注入到上下文。FileMemoryManager.read() 返回的 MemoryContent 保持该状态。

**FileMemoryManager（engine 模块）**：
- FileMemoryManager 通过 MemoryStorage 存取 Memory 实体
- `read()` 直接返回内存快照，不做额外过滤
- `append(content)` 保持 enabled=true（自 v1.0 修复，旧版曾写死为 false）
- `rewrite(content)` 保持 enabled=true
- 持久化时调用 `persist()` 写回文件
