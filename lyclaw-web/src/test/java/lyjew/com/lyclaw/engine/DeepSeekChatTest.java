...
public class DeepSeekChatTest {
    ...
    static void banner() {
        System.out.println();
        System.out.println("╔══════════════════════════════════════════════════╗");
        System.out.println("║          DeepSeek 中文对话测试                  ║");
        System.out.println("╚══════════════════════════════════════════════════╝");
        System.out.println();
    }
    ...
    void setUp() {
        ...
        ModelConfig mm = ModelConfig.builder()
                ...
                .apiKey(MINIMAX_API_KEY).model(MINIMAX_MODEL).baseUrl(MINIMAX_BASE_URL).enabled(true)
                .createdAt(LocalDateTime.now()).updatedAt(LocalDateTime.now()).build();
        ...
        sessionStorage.save(Session.builder()
                .id(sessionId).sessionId(sessionId).name("DeepSeek对话测试")
    ...
    void testSystemPrompt() throws Exception {
        ...
        String response = syncChatWithSystem(
                "你是一位资深的 Python 技术专家，回答问题时总是优先推荐 Python 解决方案。", question);

        System.out.println("🤖 AI：" + response + "\n");

        boolean mentionsPython = response.contains("Python") || response.contains("python");
        System.out.println("📊 推荐 Python: " + (mentionsPython ? "✅" : "⚠️"));
