...
/**
 * 四层记忆系统实现，模拟人类记忆的多层结构进行信息管理。
 *
 * <p>架构基于认知心理学中的 <b>Atkinson-Shiffrin 多层记忆模型</b>，
 * 将记忆划分为四个独立的存储层，每一层在信息生命周期中扮演不同角色，
 * 信息从感知层逐级向上流动：</p>
 * <ol>
 *   <li><b>感知层 (Sensory / perceptionStore)</b> — 原始输入，未经加工，
 *       默认衰减因子 0.1，直接存储 Agent 从外部环境接收到的原始数据，
 *       如用户输入文本、工具调用结果、系统事件等。这一层的数据量最大，
 *       但也最快衰减，模拟人类感官记忆的瞬时特性。</li>
 *   <li><b>短期记忆层 (Short-Term / shortTermStore)</b> — 经过提取和
 *       初步加工的精炼信息，衰减因子 0.05。当感知层数据被识别为有价值时，
 *       会被提取并提升到短期记忆层。此处的数据保留时间较长，是合并引擎的
 *       主要输入源，模拟人类工作记忆的有限容量特性。</li>
 *   <li><b>长期记忆层 (Long-Term / longTermStore)</b> — 经过合并去重后
 *       永久保存的核心知识，衰减因子 0.02（几乎不衰减）。只有重要性超过阈值
 *       且经过合并处理的信息才能进入此层。这一层的数据可供跨会话检索，
 *       模拟人类长期记忆的持久性。</li>
 *   <li><b>实体记忆层 (Entity / entityStore)</b> — 以结构化键值对形式
 *       存储的实体属性信息，按 "实体类型:实体ID" 格式组织。与前三层不同，
 *       实体记忆层存储的不是文本流，而是结构化的实体元数据（如用户偏好、
 *       对话状态、环境配置等），支持版本化管理，模拟人类对具体事物属性的
 *       类别化记忆。</li>
 * </ol>
 *
 * <p>核心数据流：原始感知 → 提取 → 短期记忆 → 合并推广 → 长期记忆。
 * 其中合并操作由 {@link lyjew.com.lyclaw.memory.consolidate.MemoryConsolidator} 负责，
 * 通过并查集算法识别语义相似的条目并合并去重；
 * 清理操作由 {@link lyjew.com.lyclaw.memory.janitor.MemoryJanitor} 负责，
 * 定期移除各层中已过期的记忆条目。</p>
 *
 * <p>所有存储层均使用 {@link ConcurrentHashMap} 以保证线程安全，
 * 适合高并发 Agent 写入场景。每个存储层都是独立的数据结构，
 * 不同层之间的数据通过显式的提升操作进行迁移，确保了数据流的单向性和可追踪性。</p>
 */
@Slf4j
@Service
