...
public class RespondStage extends PipelineStageBase {
     ...
     * 构造响应生成阶段实例。
     *
     * <p>通过 Spring 依赖注入接收 ChatFacade、ToolRegistry、ActionExecutor 和 InteractionModeProcessor。
     * ChatFacade 负责 LLM 模型的路由决策和调用，ToolRegistry 负责获取工具列表，
     * ActionExecutor 负责执行工具调用，InteractionModeProcessor 负责按名称解析交互模式引擎
     *（由 @InteractionMode 注解驱动）。
     * ChatFacade 和 InteractionModeProcessor 均为 Nullable，无 LLM 能力时退化为降级响应。</p>
     *
     * @param chatFacade               聊天门面，可为 null
     * @param toolRegistry             工具注册表，用于获取所有可用工具定义
     * @param actionExecutor           动作执行器，用于执行工具调用
     ...
     *
     * <p>构建 ToolExecutor 桥接 ActionExecutor，将工具执行委托给本地 action 服务。
    ...
    private Flux<ServerSentEvent<String>> reactWithReActEngine(ReActEngine reActEngine,
        ...
        // 构建工具执行器，通过 ActionExecutor 直接执行工具
        ToolExecutor toolExecutor = (toolName, toolCallId, arguments) -> {
            ...
            try {
                ToolExecutionResult result = actionExecutor.executeTool(
                    toolName,
                    args,
                    sandboxLevel != null ? SandboxLevel.valueOf(sandboxLevel.toUpperCase()) : SandboxLevel.DIRECT
                ).get(); // 阻塞等待结果（此处在响应式上下文中运行）
