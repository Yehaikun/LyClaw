 ...
 */
/**
 * @deprecated Since 2.0, replaced by {@link MemorySystem} which provides
 * four-layer memory architecture (Perception→ShortTerm→LongTerm→Entity).
 * Use {@code LegacyMemoryManagerAdapter} for backward compatibility.
 */
...
public interface MemoryManager {
     ...
     * 强制将内存中的变更刷入持久化存储。
     *
     * <p>由持久化决策层（{@code PersistenceExecutor}）按策略触发，
     * 不再每次 append 后自动调用。</p>
     */
    void flush();
